...
_CSS = r"""
  ...
  * { box-sizing:border-box; }
  ...
  #ladder,#heatmap { min-height:540px; } #cfiprice { min-height:330px; }
...
function confidence(d){const c=d.coverage.coverage_ratio,n=d.coverage.n_btc_positions,k=POLICY.conf;
  ...
  return['Low','var(--short)'];}
function kpiCard(label,value,sub,accent,title){return '<div class="kpi"'+(title?' title="'+title+'"':'')+
  '><span class="kpi-bar" style="background:'+(accent||'var(--hair-strong)')+'"></span><div class="kpi-label">'+
  label+'</div><div class="kpi-value">'+value+'</div><div class="kpi-sub">'+sub+'</div></div>';}
...
function renderKpis(d){
  ...
  document.getElementById('kpis').innerHTML=[
    kpiCard('Cascade Fragility Index',s.cfi.toFixed(1)+"<span class='u'>/100</span>",
      ...
      '0 = liquidations sit far from price (resilient); 100 = stacked on price (fragile). Colour = regime: calm < '+POLICY.calm+', elevated < '+POLICY.elev+', else fragile.'),
    kpiCard('Long / short asymmetry',(asym>=0?'+':'')+asym.toFixed(2),bias,biasC,
      '+1 = all near-price liquidation fuel is on the short side (squeeze-up risk); −1 = all on the long side (flush-down risk). Colour marks the exposed side.'),
    kpiCard('Liquidable within ±5%',money(w5),'±2% · '+money(w2),'',
      'Liquidable notional whose trigger sits within ±5% (and ±2%) of the current mark — the immediately-combustible fuel.'),
    ...
    kpiCard('OI captured',covPct.toFixed(0)+'%',money(cov.sampled_notional_usd)+' of '+money(m.oi_usd)+' OI','',
      'Share of total reported BTC open-interest notional our wallet sample currently represents.'),
    kpiCard('Signal confidence',cf[0],covPct.toFixed(0)+'% coverage · '+Math.round(cov.n_btc_positions)+' positions',cf[1],
      'How much to trust this reading. High = ≥'+Math.round(k.highCov*100)+'% coverage & ≥'+k.highN+' positions; Medium = ≥'+Math.round(k.medCov*100)+'% & ≥'+k.medN+'; else Low. Colour = green/amber/red.'),
  ].join('');
...
function renderLadder(d){
  ...
  const lay=baseLayout('Liquidation '+(bars?'notional':'density')+' by price level — where open BTC leverage triggers');
  lay.height=540; lay.hovermode='y unified'; if(bars) lay.barmode='overlay';
...
function renderCfiPrice(d){
  ...
  const lay=baseLayout('Cascade Fragility Index vs BTC price — over time');
  lay.height=330; lay.margin={l:54,r:62,t:46,b:34}; lay.hovermode='x unified'; lay.showlegend=true;
  lay.legend={orientation:'h',y:1.12,x:0,bgcolor:'rgba(0,0,0,0)',font:{size:11,color:MUTED}};
...
function renderHeatmap(d){
  ...
  const lay=baseLayout('Liquidation heatmap — '+per+' liquidable notional by price (white = mark path)');
...
function renderAll(d){
  ...
  renderKpis(d); renderLadder(d); renderHeatmap(d); renderCfiPrice(d); renderQuality(d); setFresh();
...
<figure class="chart reveal" style="animation-delay:.14s">
  <div id="ladder-wrap">
    ...
    <p class="explain">Where the open BTC leverage we track would be <b>force-liquidated</b>, by price.
       ...
       <b>bars</b> shows the exact per-level notional. Hover any level for the amount; drag/scroll to zoom.</p>
    <div class="legend">
...
<figure class="chart reveal" style="animation-delay:.20s">
  <figcaption><span>Fig.02 — Fragility &amp; price over time</span><b>CFI vs BTC</b></figcaption>
  <p class="explain">The <b>Cascade Fragility Index</b> (gold, left axis) against <b>BTC price</b> (right axis).
     Green / amber / red bands mark calm / elevated / fragile regimes — watch fragility build or bleed off as
     price moves. Drag/scroll to zoom.</p>
  <div id="cfiprice"></div>
</figure>
"""
